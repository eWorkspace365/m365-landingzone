...
Write-Host "Retrieving role assignments for all resources in Azure subscriptions..." -ForegroundColor DarkGray
foreach ($subscription in $subscriptions) {
    ...
    foreach ($resource in $resources) {
        ...
        # Filter role assignments that are not inherited
        $nonInheritedAssignments = $roleAssignments | Where-Object { $_.IsInherited -eq $false }

        # Add the non-inherited role assignments to the results array
        foreach ($roleAssignment in $nonInheritedAssignments) {
...
$HTMLTable = $results | ConvertTo-Html -Head $CSSStyle -Title "Azure Resource Role Assignments Report" | Out-String

$htmlContent = @"
<h2>Azure Resource Role Assignments Report</h2>
...
<p>This report lists all non-inherited role assignments for resources in your Azure environment.</p>
...
$params = @{
    message = @{
        subject = "Azure Resource Role Assignments Report"
        body = @{
            contentType = "HTML"
            content = $htmlContent
        }
