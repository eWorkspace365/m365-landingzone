...
# Definieer een functie om vreemde karakters te verwijderen
function Remove-NonAlphaNumericCharacters {
    param($string)

    $regex = '[^a-zA-Z0-9\s]+'
    return [regex]::Replace($string, $regex, '')
}
...
### Maak het hooftobject

$outputObj = New-Object PSCustomObject

# Voeg "scores" object toe aan hooftobject
$outputObj | Add-Member -MemberType NoteProperty -Name OverallScores -Value $OverallScores -force
$outputObj | Add-Member -MemberType NoteProperty -Name AppsScores -Value $AppsScores -force
$outputObj | Add-Member -MemberType NoteProperty -Name DeviceScores -Value $DeviceScores -force
$outputObj | Add-Member -MemberType NoteProperty -Name DataScores -Value $DataScores -force
$outputObj | Add-Member -MemberType NoteProperty -Name IdentityScores -Value $IdentityScores -force

### CREATE DUMP FILE
Write-Output "Creating local file"
SaveObjectToJsonFile -klantnaam $Customer -filename "secure-score.json" -itemToSave $outputObj

# Update PowerShell-script zichzelf
Write-Host "Graph scans zijn uitgevoerd."
